...
jira_re = 'VSUIP-\d*|VUA-\d*|VUD-\d*|VUCL-\d*|VUGV-\d*|VUHP-\d*|VUL-\d*|VUN-\d*|VUSP-\d*|VUVC-\d*|VSUN-\d*|VUS-\d*|VUSS-\d*|' \
          'VUVCHA-\d*|VUCI-\d*|VUVG-\d*|VSUIR-\d*|MU-\d*|VPAR-\d*'
...
try:                             # Catch exceptions with try/except
    ...
    with open('../data/intermediate/h5c-main/vsphere-client-modules.csv') as csvfile:
    ...
    for changelist in changelists:
        ...
        print(bug_number_index)
        print(reviewed_by_index)
        if bug_number_index > -1 and reviewed_by_index > -1:
            bugzilla_bug = comments[bug_number_index+11:reviewed_by_index - 1]
            bugzilla_bug.strip()
            print(bugzilla_bug)
    ...
    # Write to a file
    wtr = csv.writer(open('../data/intermediate/h5c-main/vsphere-client-modules-bug-list.csv', 'w'))
